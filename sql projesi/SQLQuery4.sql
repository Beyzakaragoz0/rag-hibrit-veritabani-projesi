USE RagHybridDB;
GO

CREATE OR ALTER FUNCTION dbo.fn_VectorSimilarity (
    @Vector1 NVARCHAR(MAX),
    @Vector2 NVARCHAR(MAX)
)
RETURNS FLOAT
AS
BEGIN
    DECLARE @DotProduct FLOAT = 0, @Mag1 FLOAT = 0, @Mag2 FLOAT = 0;

    SELECT 
        @DotProduct = SUM(CAST(v1.value AS FLOAT) * CAST(v2.value AS FLOAT)),
        @Mag1 = SQRT(SUM(SQUARE(CAST(v1.value AS FLOAT)))),
        @Mag2 = SQRT(SUM(SQUARE(CAST(v2.value AS FLOAT))))
    FROM OPENJSON(@Vector1) v1
    JOIN OPENJSON(@Vector2) v2 ON v1.[key] = v2.[key];

    IF @Mag1 * @Mag2 = 0 RETURN 0;
    RETURN @DotProduct / (@Mag1 * @Mag2);
END;
GO

CREATE OR ALTER PROCEDURE dbo.sp_RAG_AramaYap
    @AramaMetni NVARCHAR(MAX),
    @KullaniciID INT
AS
BEGIN
    SET NOCOUNT ON;

    INSERT INTO dbo.SearchLogs (UserID, SearchQuery)
    VALUES (@KullaniciID, @AramaMetni);

    SELECT 
        dc.ChunkID,
        c.CategoryName,
        d.Title,
        dc.ChunkText,
        ISNULL((
            SELECT STRING_AGG(t.TagName, ', ')
            FROM dbo.DocumentTags dt
            JOIN dbo.Tags t ON dt.TagID = t.TagID
            WHERE dt.DocumentID = d.DocumentID
        ), 'Etiket Yok') AS Etiketler
    FROM dbo.Documents d
    JOIN dbo.Categories c ON d.CategoryID = c.CategoryID
    JOIN dbo.DocumentChunks dc ON d.DocumentID = dc.DocumentID
    WHERE dc.ChunkText LIKE '%' + @AramaMetni + '%'
       OR d.Title LIKE '%' + @AramaMetni + '%';
END;
GO

CREATE OR ALTER PROCEDURE dbo.sp_SistemOzeti
AS
BEGIN
    SET NOCOUNT ON;

    SELECT 
        (SELECT COUNT(*) FROM dbo.Documents) AS ToplamBelge,
        (SELECT COUNT(*) FROM dbo.DocumentChunks) AS ToplamChunk,
        (
            SELECT TOP 1 c.CategoryName
            FROM dbo.Documents d
            JOIN dbo.Categories c ON d.CategoryID = c.CategoryID
            GROUP BY c.CategoryName
            ORDER BY COUNT(*) DESC
        ) AS EnYogunKategori,
        (SELECT COUNT(*) 
         FROM dbo.SearchLogs 
         WHERE SearchDate > DATEADD(DAY, -1, GETDATE())) AS Son24SaatTrafik;
END;
GO